...
class TimeSelectionBB(TimeSelection):
    ...
    def fixSelections(self):
        """
        Improves the timeselection by combining multiple detectors with the
        highest significance in the data
...
class TimeSelectionNew(TimeSelection):
    def __init__(self, **kwargs):
        ...
        self._replace_n5 = kwargs.get("replace_n5", True)

        logger.info(f"Starting the TimeSelectionNew with following kwargs: {kwargs}")
        ...
        while self._bkg_pos_start - self._active_time_stop < 20:
            logger.info(f"Background pos start too close to trigger time, rerunning")
        ...
        while flag:
            ...
            if (
                ...
                if self._min_bb_block_bkg_duration > 0.512 and too_small_bin:
                    logger.debug("Decreasing min bkg bb block duration")
                    ...
                    if len(self._bb_indices_self[mask][self._pos_bins]) == 0:
                        if self._trigger_zone_background_stop - 0.512 > 1.024:
                            logger.warning("Decreasing bkg pos start time!")
                            logger.warning(
                    ...
                    if len(self._bb_indices_self[mask][self._pos_bins]) == 0:
                        if self._trigger_zone_background_start + 0.512 > -1.024:
                            logger.warning("Decreasing bkg neg stop time!")
                            logger.warning(
                                "This may have negative effect on the TimeSelection"
                            )
    ...
    def _select_active_time(self):
        ...
        for d in lu:
            temp = self._tr.time_series[d].significance_per_interval.copy()
        ...
        n5_skipped = False
        if self._replace_n5:
            logger.info("Checking if n5 is used for timeselection, if so replacing it")
            for k, v in avgs_sorted[-3:]:
                if k != "n5":
                    logger.debug(f"Using {k}")
                    obs_significance += self._tr._rates[:, name2id(k), :].reshape(
                        len(self._tstart), 8
                    )
                else:
                    logger.debug("skipping n5")
                    n5_skipped = True
            if n5_skipped:
                k, v = avgs_sorted[-4]
                logger.info(f"Using {k} instead of n5")
                obs_significance += self._tr._rates[:, name2id(k), :].reshape(
                    len(self._tstart), 8
                )
        else:
        ...
        logger.debug(f"Active Time eneded because of {reason}")
    ...
    def _create_additional_timeseries(self, cps):
        ...
        logger.debug(f"This is the significance {sig}")
