...
class HealpixMap(object):
    ...
    def __init__(self, analysis_results, nside=32, error=1, n_samples_point=100):
        ...
        self._point_source_name = (
            analysis_results.optimized_model.get_point_source_name(0)
        )
    ...
    def _get_systematic_samples(self, mean):  # error in deg
        sample_angles = np.random.multivariate_normal(
            [mean[0], mean[1]],
            [
                [np.power(np.cos(np.deg2rad(mean[1])), -2) * (self._error) ** 2, 0.0],
                [0.0, (self._error) ** 2],
            ],
...
def healpix_with_sys(
    nside=512, n_samples_point=100, sat_phi=0, result_path=None, save_path=None
):
    """
    Get healpix with no sys error
    """
    ...
    results = load_analysis_results(result_path)
    healpix = HealpixMap(
        results, nside=nside, error=error, n_samples_point=n_samples_point
    )
    healpix.write_map(save_path)
